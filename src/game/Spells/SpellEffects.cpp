...
void Spell::EffectDummy(SpellEffectIndex eff_idx)
{
    ...
    case SPELLFAMILY_GENERIC:
    {
        ...
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->CastSpell(m_caster, (urand(0, 1) ? (m_caster->getGender() == GENDER_MALE ? 8219 : 8220) // Flip Out - ninja
                                                           : (m_caster->getGender() == GENDER_MALE ? 8221 : 8222)),
                                    TRIGGERED_OLD_TRIGGERED, nullptr); // Yaaarrrr - pirate
            ...
            if (m_triggeredByAuraSpell && unitTarget)
            {
                ...
                case 28821: // Lightning Shield
                {
                    ...
                    for (const auto &aura : auras)
                    {
                        ...
                        if (spell->SpellFamilyName == SPELLFAMILY_SHAMAN && spell->SpellFamilyFlags & uint64(0x0000000000000400))
                            return;
                    }
        ...
        case 22659: // Spawn Drakonid
        {
            ...
            {
            case 14312:
                spellSummonDrakonid = 22658;
                break; // Blue
            case 14310:
                spellSummonDrakonid = 22656;
                break; // Green
            case 14311:
                spellSummonDrakonid = 22657;
                break; // Bronze
            case 14309:
                spellSummonDrakonid = 22655;
                break; // Red
            case 14307:
                spellSummonDrakonid = 22654;
                break; // Black
            default:
                return;
            }
        ...
        case 23173: // Brood Affliction - TODO: Rework to use proper targeting
        {
            ...
            for (auto unit : fifteenTargets)
            {
                ...
                if (unit->HasAura(23153, EFFECT_INDEX_0) && unit->HasAura(23154, EFFECT_INDEX_0) && unit->HasAura(23155, EFFECT_INDEX_0) && unit->HasAura(23170, EFFECT_INDEX_0) && unit->HasAura(23169, EFFECT_INDEX_0))
                {
                    unit->RemoveAllAuras();
    ...
    case SPELLFAMILY_HUNTER:
    {
        ...
        case 23989: // Readiness talent
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                auto cdCheck = [](SpellEntry const &spellEntry) -> bool
                { return (spellEntry.SpellFamilyName == SPELLFAMILY_HUNTER && spellEntry.Id != 23989 && GetSpellRecoveryTime(&spellEntry) > 0); };
                static_cast<Player *>(m_caster)->RemoveSomeCooldown(cdCheck);
            }
    ...
    case SPELLFAMILY_SHAMAN:
    {
        if (m_spellInfo->SpellFamilyFlags & uint64(0x0000000000200000)) // Flametongue Weapon Proc, Ranks
        {
            if (m_CastItem)
            {
                ...
                int32 bonusDamage = m_caster->SpellBaseDamageBonusDone(GetSpellSchoolMask(m_spellInfo)) + unitTarget->SpellBaseDamageBonusTaken(GetSpellSchoolMask(m_spellInfo));
...
void Spell::EffectTriggerSpell(SpellEffectIndex effIndex)
{
    ...
    case TARGET_LOCATION_UNIT_MINION_POSITION:
        break; // confirmed by 31348 nothing is forwarded
    default:
    ...
    for (int32 count = 0; count < amount; ++count)
    {
        ...
        if (Creature *summon = WorldObject::SummonCreature(
                TempSpawnSettings(m_trueCaster, creature_entry, px, py, pz, m_trueCaster->GetOrientation(), summonType, m_duration, false,
                                  IsSpellSetRun(m_spellInfo), 0, 0, 0, false, false, m_spellInfo->Id, -1, level),
                m_trueCaster->GetMap()))
        {
...
void Spell::EffectWeaponDmg(SpellEffectIndex eff_idx)
{
    ...
    case SPELL_DAMAGE_CLASS_MAGIC:
        break; // TODO: Add behaviour for spell behaviour
...
void Spell::EffectSkinning(SpellEffectIndex /*eff_idx*/)
{
    ...
    if (!loot)
    {
        ...
        int32 reqValue = targetLevel < 10 ? 0 : targetLevel < 20 ? (targetLevel - 10) * 10
                                                                 : targetLevel * 5;
...
void Spell::EffectSummonDemon(SpellEffectIndex eff_idx)
{
    ...
    for (int i = 0; i < m_currentBasePoints[eff_idx]; i++)
    {
        Creature *summon = WorldObject::SummonCreature(TempSpawnSettings(m_caster, m_spellInfo->EffectMiscValue[eff_idx], x, y, z, m_caster->GetOrientation(), TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, 3600000, false,
                                                                         IsSpellSetRun(m_spellInfo), 0, 0, false, false, m_spellInfo->Id),
                                                       m_caster->GetMap());
